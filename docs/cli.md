...
with the `Generated by AgtMLS` header. For Aider, removing `CONVENTIONS.md`
also removes the `read:` block install appended to `.aider.conf.yml`, and the
file too if nothing else is in it.
