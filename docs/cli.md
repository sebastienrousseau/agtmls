...
stays unless you pass `--remove-prompt`, and even then only if it still begins
with the `Generated by AgtMLS` header. For Aider, install adds
`CONVENTIONS.md` to the top-level `read:` list of `.aider.conf.yml`, joining a
list already there; removing `CONVENTIONS.md` takes that entry out again, and
the file too if nothing else is in it.
...
### Does the agent actually read the skills?

The lockfile proves the installed bytes; it cannot see an agent skip a skill.
`verify <agent> --target <repo> --live` asks the agent itself which skills it
loads there, and fails (exit `1`) naming each installed skill it does not
list, or saying why the agent could not be asked:

- `claude`: starts one headless session and reads the `skills` list of its
  `system/init` event, stopping it as soon as that event arrives, so at most
  one short request is started.
- `codex`: renders the prompt Codex would send (`codex debug prompt-input`)
  without calling the model, and reads its skill list.

`doctor` also checks each agent's user-level skill directories
(`~/.claude/skills`, `~/.codex/skills`), which no lockfile covers: a link
there that no longer resolves is a `WARN` naming it, because the agent skips
it without a word. Real directories and dotfiles there are the user's own and
are not judged. Which directories and which live check belong to which agent
is data in `providers.json` (`user_skills_dirs`, `live_probe`).
