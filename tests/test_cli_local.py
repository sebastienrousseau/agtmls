...
class UninstallTests(CliFixture):
    ...
    def test_uninstalling_one_agent_keeps_the_lockfile_the_others_still_use(self) -> None:
        """One lockfile serves every agent installed in a target. Uninstalling
        codex from a target that also had claude deleted it, so claude's
        skills stayed with no record: verify failed and a later uninstall
        removed 0 items."""
        codex = self.target / ".codex" / "skills"
        codex.mkdir(parents=True)
        for name in GENERAL:
            shutil.copytree(self.fixture / "skills" / name, self.skills / name)
            shutil.copytree(self.fixture / "skills" / name, codex / name)
        lock = self.record(GENERAL, "copy")

        code, output = capture(self.cli.uninstall, self.target, "codex", False)
        self.assertEqual(code, 0, output)
        self.assertFalse(codex.exists(), "codex's skills survived its uninstall")
        self.assertTrue(lock.exists(), "the lockfile went while claude still uses it")
        self.assertEqual(self.cli.lockfile.verify(self.target, self.skills), [])

        code, output = self.uninstall()
        self.assertEqual(code, 0, output)
        self.assertFalse(lock.exists(), "the lockfile outlived the last agent using it")
        self.assertIn(f"removed {len(GENERAL) + 1} AgtMLS-managed item(s)", output)

    def test_removing_aiders_prompt_also_removes_its_registration(self) -> None:
        """install registers CONVENTIONS.md in .aider.conf.yml; removing the
        prompt left Aider told to read a file that was gone. Only the block
        install appended goes; the user's own settings stay."""
        conventions = self.target / "CONVENTIONS.md"
        conf = self.target / ".aider.conf.yml"
        block = "read:\n  - CONVENTIONS.md\n"
        cases = {
            block: None,
            "model: sonnet\n" + block: "model: sonnet\n",
        ...
        for before, after in cases.items():
            with self.subTest(before=before):
                conventions.write_text("<!-- Generated by AgtMLS -->\n", encoding="utf-8")
                conf.write_text(before, encoding="utf-8")
                code, output = capture(self.cli.uninstall, self.target, "aider", True)
                self.assertEqual(code, 0, output)
                self.assertFalse(conventions.exists())
                if after is None:
                    self.assertFalse(conf.exists(), "a config holding only our block was left")
                else:
                    self.assertEqual(conf.read_text(encoding="utf-8"), after)
        conf.write_text(block, encoding="utf-8")
        capture(self.cli.uninstall, self.target, "aider", False)
        self.assertTrue(conf.exists(), "the registration went while CONVENTIONS.md stayed")
        conf.unlink()
        conventions.write_text("<!-- Generated by AgtMLS -->\n", encoding="utf-8")
        code, output = capture(self.cli.uninstall, self.target, "aider", True)
        self.assertEqual(code, 0, output)
        self.assertIn("removed 1 AgtMLS-managed item(s)", output, "a config that was never there was counted")
