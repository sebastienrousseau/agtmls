...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='agtmls-evolve-') as td:
        ...
        p=subprocess.run([sys.executable,str(ROOT/'scripts'/'evolve-session.py'),str(tr),'--skill-name','Port Helper','--out-dir',str(t/'evo')],text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,cwd=ROOT,check=False)
        ...
        e=subprocess.run([sys.executable,str(ROOT/'scripts'/'record-evidence.py'),'--skill','cross-language-port','--out-dir',str(t/'runs'),'--command','pytest','--file','x.py'],text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,cwd=ROOT,check=False)
