...
description: YAML 1.2.2 spec knowledge as it applies to noyalib — the domain-theory pack for scalar resolution (the resolve_plain_ext table), documents/streams/directives (§6.8, §9.1.2), block vs flow, block-scalar chomping (|/>/-/+), line breaks (§5.4 LF/CR/CRLF), anchors/aliases/merge keys with DoS budgets (max_alias_expansions, alias_anchor_ratio, max_merge_keys), tags (!!str/!!int/!!bool/!!null/!!binary → RFC 4648), YAML 1.1 vs 1.2 flags (legacy_booleans, legacy_octal_numbers, legacy_sexagesimal), the Norway problem, MergeKeyPolicy Auto/AsOrdinary/Error, and the official YAML Test Suite conformance arithmetic. Load when reasoning about YAML semantics, plain-scalar type coercion, spec-conformance work, or any question a reader without the 1.2.2 spec loaded in their head would struggle with. Verb-form triggers — use for "explain YAML merge keys / anchors / chomping / tags", "how do I resolve a plain scalar", "what does YAML 1.2 say about block scalars", "why does `no` parse as false", "which YAML feature causes the Norway problem", "does noyalib accept `nan` / `.inf` / octal `0o7`", or "how many cases does the official YAML suite cover".
date: 2026-07-08
---
